...
def inspect(root, config):
    ...
    for key, required in (('required_tools', True), ('optional_tools', False)):
        for name in config.get(key, []):
            results.append({'name': name, 'status': 'available' if shutil.which(name) else 'missing',
                            'required': required, 'authentication': 'not checked'})
    for name in config.get('required_files', []):
        path = root / name
        present = path.is_file() and path.resolve().is_relative_to(root.resolve())
        results.append({'name': name, 'status': 'present' if present else 'missing', 'required': True})
    return results
...
def main():
    ...
    try:
        ...
        results = inspect(root, json.loads((root / '.workflow/project.json').read_text()))
    except (OSError, ValueError) as exc:
        parser.error(str(exc))
    if args.json:
        print(json.dumps({'capabilities': results, 'credentials': 'not checked',
                          'verification': 'not run', 'independent_review': 'not run'}, indent=2))
    else:
        for result in results:
            requirement = 'required' if result['required'] else 'optional'
            print(f"{result['status']:9} {result['name']} ({requirement})")
        print('Tool presence does not verify credentials, daemon health, tests, or independent review.')
